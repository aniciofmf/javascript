...
function minUmbrellas(weather) {
	...
	if (weather.length <= 1 || isAJourneyToNotUseUmbrella(weather)) return 0;

	if (isAJourneyToUseUmbrella(weather)) return 1;

	for (let i = 0; i < weather.length; i++) {
		if (useUmbrella(weather[i])) {
			if (i % 2 === 0) {
				if (!umbrellaAtHome) {
					umbrellaAtWork++;
				} else {
	...
	/////////////////////////////////
	function isAJourneyToNotUseUmbrella(journey) {
		return journey.every((day) => withoutUmbrella.includes(day));
	}

	function isAJourneyToUseUmbrella(journey) {
		return journey.every((day) => withUmbrella.includes(day));
	}

	function useUmbrella(weather) {
		return withUmbrella.includes(weather);
	}
}
